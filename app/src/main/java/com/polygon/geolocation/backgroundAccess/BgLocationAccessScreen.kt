...
package com.polygon.geolocation.backgroundAccess
...
private fun BackgroundLocationControls() {
	...
	val controlsState = remember(workerState) {
		...
		if (enqueued) {
			ControlsState(
				text = "Check the logcat for location updates every 15 sec",
			...
			ControlsState(
				...
				onClick = {
					// Schedule a periodic worker to check for location every 15 sec
